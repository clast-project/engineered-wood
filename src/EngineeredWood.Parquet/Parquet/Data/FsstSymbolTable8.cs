...
/// <summary>
/// The 8-bit code variant of <see cref="FsstSymbolTable"/> — the proposal's
/// <c>SymbolTableType.FSST</c>.
/// </summary>
/// <remarks>
...
/// 8-bit training algorithm emits codes in gain order, so <see cref="TryTrain"/> renumbers
/// them and carries a remap that <see cref="Compress"/> applies to every emitted code.</para>
...
/// </remarks>
internal sealed class FsstSymbolTable8 : FsstSymbolTable
{
...
#pragma warning disable EWPARQUET0003 // The enum value names a wire constant; writing one is the caller's opt-in.
    public override SymbolTableType Type => SymbolTableType.Fsst;
#pragma warning restore EWPARQUET0003
    ...
    /// </summary>
    public static FsstSymbolTable8? TryTrain(ReadOnlySpan<byte[]> values)
    {
        if (values.Length == 0)
            return null;
        ...
        return new FsstSymbolTable8(lengths, symbols, trained, remap);
    }
    ...
    /// to check.
    /// </summary>
    public static FsstSymbolTable8 Parse(ReadOnlySpan<byte> body)
    {
        ...
        return new FsstSymbolTable8(lengths, symbols, trained: null, remap: null);
    }

    /// <inheritdoc/>
    /// <remarks>Every byte escaping to a two-byte sequence (§5.3).</remarks>
    public override long MaxCompressedLength(long rawLength) => rawLength * 2;
    ...
    public override int MaxDecompressedLength(int compressedLength) =>
        Clast.Fsst.FsstDecoder.MaxDecompressedLength(compressedLength);
    ...
    private Clast.Fsst.FsstDecoder Decoder =>
